// importing the function from DataService

const { appendIndex } = require('./DataService');
...
//a. Append Index to Object

const Records = appendIndex(mockdata);
  ...
  // b. If we pass postId as 1 it should give us all comments within that postId
function getRecordsByPostId(postId) {
    const Records = mockdata.filter(record => record.postId === postId);
    return Records;
  }
  
  const postIdToSearch = 1;
  const records = getRecordsByPostId(postIdToSearch);
  //console.log(records);




// c. Group all records using postId 

  function groupRecordsByPostId(records) {
    const group = {};
    records.forEach(record => {
      ...
      if (!group[postId]) {
        ...
        group[postId].push(record);
      }
    });
    return group;
  }
  
  const groupedRecords = groupRecordsByPostId(mockdata);
  console.log(groupedRecords);




// d. Total number of comments in each post 
  ...
  function TotalRecords(records) {
    ...
    records.forEach(record => {
      ...
      if (!grouped[postId]) {
        ...
        grouped[postId].records.push(record);
        grouped[postId].TotalRecords += 1;
      }
    });
    return grouped;
  }
  
  const groupedrecords = TotalRecords(mockdata);
  //console.log(groupedrecords);



  
// e. List records having email containing substring "Mathias"

  function Substring(data, substring) {
    const filterRecords = data.filter(record => record.email.includes(substring));
    return filterRecords;
  }
  
  const substringToSearch = "Mathias";
  const requiredsubstring = Substring(mockdata, substringToSearch);
  //console.log(requiredsubstring);



// f. List of emailId ( unique )

  function getUniqueEmailIds(data) {
    const uniqueEmails = new Set();
    data.forEach(record => uniqueEmails.add(record.email));
    return Array.from(uniqueEmails);
  }
  
  const uniqueEmailIds = getUniqueEmailIds(mockdata);
